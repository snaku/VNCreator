...
{
	VNCreatorApp::queueFamilyIndices familyIndices = findQueueFamilies(device);
	...
	if (extSupported)
	{
		VNCreatorApp::swapChainSupport swapchainSupport = swapchain.querySwapChainSupport(device, *this);
...
VNCreatorApp::queueFamilyIndices VulkanInit::findQueueFamilies(VkPhysicalDevice device)
{
	VNCreatorApp::queueFamilyIndices familyIndices;
...
{
	VNCreatorApp::queueFamilyIndices familyIndices = findQueueFamilies(physicalDevice);
...
std::vector<VkDeviceQueueCreateInfo> VulkanInit::processQueueFamilies(const VNCreatorApp::queueFamilyIndices& indices) const {
