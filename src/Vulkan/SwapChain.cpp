...
swapChainSupport Swapchain::querySwapChainSupport(VkPhysicalDevice device, VulkanInit& vulkan)
{
...
VkPresentModeKHR Swapchain::swapPresMode(const std::vector<VkPresentModeKHR>& availPresModes)
{
	for (const auto& mode : availPresModes)
	{
	...
	// sinon VK_PRESENT_MODE_FIFO_KHR
...
{
	swapChainSupport swapChainSupport = querySwapChainSupport(vulkan.getPhysicalDevice(), vulkan);

	VkSurfaceFormatKHR surfaceFormat = swapSurfaceFormat(swapChainSupport.formats);
	VkPresentModeKHR presMode = swapPresMode(swapChainSupport.presModes);
	...
	queueFamilyIndices familyIndices = vulkan.findQueueFamilies(vulkan.getPhysicalDevice());
...
{
	this->swapChainImgView.resize(this->swapChainImg.size());
